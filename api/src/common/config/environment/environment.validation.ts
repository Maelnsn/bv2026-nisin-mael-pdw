...
const corsOriginsSchema = nonEmptyStringSchema.transform((value, context) => {
  const origins = value
    .split(',')
  ...
  if (origins.length === 0 || origins.includes('*')) {
    context.addIssue({
      ...
      message: 'APP_CORS_ORIGIN must be an explicit allowlist and cannot use *',
    });
    return z.NEVER;
  }
  return origins;
...
const environmentSchema = z
  .object({
    ...
    APP_BASE_URL: nonEmptyStringSchema
    ...
    SWAGGER_PATH: nonEmptyStringSchema
      .regex(/^[a-zA-Z0-9][a-zA-Z0-9/_-]*$/)
      .transform((value) => value.replace(/^\/+|\/+$/g, '')),
